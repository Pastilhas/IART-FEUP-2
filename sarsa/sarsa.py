...
class Sarsa:
    def __init__(self, actions, alpha=0.2, gamma=0.9, epsilon=0.2):
        ...
        # Learning rate. Higher value means new value is less important.
        self.alpha = alpha
        # Discount factor. Higher value means focus on future reward.
        self.gamma = gamma
        # Exploration factor. Higher means more exploration.
    ...
    def saveTo(self, filename, settings):
        ...
        for row in settings:
            File.write("N=" + str(row[0]) + " | alpha=" + str(row[1]) +
                       " | gamma=" + str(row[2]) + " | epsilon=" + str(row[3]) + " |\n")
        File.write("\nQ-Table\n")
        table = {}
        for key in self.q_table:
            if key[0] in table:
                table[key[0]][key[1]] = float(
                    "%.2f" % self.q_table.get(key, 0.0))
            else:
